...
function extractEventId(filePath) {
  ...
  const otherMatches = filePath.match(
    /(?:event_quest|event_story_s|event_boss_battle|panel_(?:bg|mission_(?:large|small)_banner))_(\d+)\.png$/
  );
...
export default async function generateEventQuests() {
  ...
  const logoFiles = fileList.filter(
    (line) =>
      line.includes("/AllParticipationEventLogo/") && line.includes("event_quest_logo_") && line.endsWith(".png")
  );
